...
const ACCOUNT_NAV = [
  { label: "Hồ sơ của tôi", to: "/account/profile", icon: "🧑" },
  { label: "Việc làm đã lưu", to: "/account/saved-jobs", icon: "♡" },
];

const EMPLOYER_NAV = [
  { label: "Đăng tin tuyển dụng", to: "/jobs/create", icon: "➕" },
];
...
function UserLayout() {
  const { role } = useAuth();
  const navItems = role === "EMPLOYER" || role === "ADMIN" ? [...ACCOUNT_NAV, ...EMPLOYER_NAV] : ACCOUNT_NAV;
