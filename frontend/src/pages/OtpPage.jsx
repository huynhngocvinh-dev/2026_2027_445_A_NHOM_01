...
function OtpPage() {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Không thể kết nối đến server");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="relative z-10 w-full min-h-screen flex flex-col items-center justify-center px-12">
          <div className="flex items-center gap-2 mb-8">
            <div className="w-10 h-10 bg-white rounded-xl shadow-md flex items-center justify-center">
              <FiBriefcase className="text-blue-700" size={20} />
            </div>
            <span className="text-2xl font-bold text-blue-900">JobFinder</span>
          </div>

          <div className="w-full max-w-sm bg-white rounded-xl shadow-xl p-7">
            <h2 className="text-2xl font-bold text-blue-900 text-center">Đăng ký và Xác thực OTP</h2>
            <p className="mt-4 text-sm text-gray-500 text-center leading-relaxed">
              Kết nối nhân tài công nghệ với những cơ hội hàng đầu. Xác thực email để hoàn tất
              đăng ký và bắt đầu hành trình sự nghiệp của bạn.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-1/2 min-h-screen flex items-center justify-center bg-[#f8f9ff] px-6 sm:px-12">
        <div className="w-full max-w-md">
          <div className="mb-7">
            ...
            {email && <p className="mt-2 text-sm font-semibold text-gray-700">{email}</p>}
          </div>
          ...
          <form onSubmit={handleVerify}>
            <div className="flex justify-between gap-2">
              {otp.map((digit, index) => (
                <input
                  ...
                  className="w-11 h-11 sm:w-12 sm:h-12 text-center text-lg font-semibold bg-white border border-gray-300 rounded-lg outline-none transition focus:border-blue-600 focus:ring-2 focus:ring-blue-100"
                />
              ))}
            </div>
            ...
            <div className="mt-5 text-center text-xs text-gray-500">
              Chưa nhận được mã?{" "}
              {countdown > 0 ? (
                <span className="text-blue-600 font-medium">Gửi lại sau {countdown}s</span>
              ) : (
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-11 mt-5 rounded-full bg-blue-900 hover:bg-blue-950 text-white text-sm font-semibold transition disabled:opacity-60 disabled:cursor-not-allowed"
            >
          ...
          <button
            ...
            className="w-full mt-5 text-sm text-gray-500 hover:text-blue-700 transition"
          >
